---
title: "Visualise Singapore Merchandise Trade Performances"
subtitle: "From Singapore Financial Year 2019 to 2022"
editor: visual
date:  18 Feb 2023
...
This post explores Singapore trade performances visualiation from different perspectives.

A government spending with budget deficit can reduce the national savings rate and raises the trade deficit, and BOT, theorectically can affect a country's GDP, currency exchange rate, level of inflation as well as interest rates. Hence, I thought it would be sensible not to exclude exploration based on financial year instead of specific period.

Based on Singapore Ministry of Finance, the Singapore Budget is prepared based on the [Singapore financial year](https://www.mof.gov.sg/singaporebudget/about-budget#:~:text=The%20Singapore%20Budget%20is%20prepared,31%20March%20the%20following%20year.) which begins 1 April and ends on 31 March.

The focuses for this study is still the same as the submitted Take-home Exercise 4, i.e. explore Singapore's trade performances :
...
The data used in this study is the same data used in **Take-home Exercise 4.1.**

## **3.2 Import Data**

### 3.2.1 Read T1 as import dataset

```{r}
#| code-fold: false
import2 <- read_excel("data/merchandiseTrade_region.xlsx",
                           sheet = "T1",
...
### 3.2.2 Read T1 as export dataset

```{r}
#| code-fold: false
export2 <-  read_xlsx("data/merchandiseTrade_region.xlsx",
                           sheet = "T2",
...
#### 3.3.1.1 For *trade_import2* dataframe

```{r}
trade_import2 <- import2 %>%
  separate(`Data Series`, c("country", "unit"),
           sep = ' \\(') %>%
  mutate(unit = str_remove(unit, '\\)'))
```

#### 3.3.1.2 For *trade_export2* dataframe

```{r}
trade_export2 <- export2 %>%
  separate(`Data Series`, c("country", "unit"),
           sep = ' \\(') %>%
  mutate(unit = str_remove(unit, '\\)'))
```
...
#### 3.3.2.1 For *trade_imports2* dataframe

```{r}
#| code-fold: false
trade_imports2 <- trade_import2 %>%
  slice(-26, -70)
```
...
```{r}
#| code-fold: false
trade_importsK2 <- trade_imports2 %>%
...
```{r}
#| code-fold: false
trade_importsM2 <- trade_imports2 %>%
...
```{r}
#| code-fold: false
trade_exportsK2 <- trade_exports2 %>%
  filter(str_detect(unit, "Thousand")) %>%
  select(-(unit))
```
...
```{r}
#| code-fold: false
trade_exportsM2 <- trade_exports2 %>%
  filter(str_detect(unit, "Million")) %>%
  select(-(unit))
```
...
importK_pvL2 <- importK_pvL2 %>%
  mutate(month = factor(month
                        (importK_pvL2$`period`),
                        levels = 1:12, 
                        labels = month.abb, 
                        ordered = TRUE)) %>%
  mutate(year = as.character(year(ymd(importK_pvL2$`period`)))) %>%
  relocate(year, month) %>%
  select(-temporary)
```
...
importM_pvL2 <- importM_pvL2 %>%
  mutate(month = factor(month
                        (importM_pvL2$`period`),
                        levels = 1:12, 
                        labels = month.abb, 
                        ordered = TRUE)) %>%
  mutate(year = as.character(year(ymd(importM_pvL2$`period`)))) %>%
  relocate(year, month) %>%
  select(-temporary)
```
...
exportK_pvL2 <- exportK_pvL2 %>%
  mutate(month = factor(month
                        (exportK_pvL2$`period`),
                        levels = 1:12, 
                        labels = month.abb, 
                        ordered = TRUE)) %>%
  mutate(year = as.character(year(ymd(exportK_pvL2$`period`)))) %>%
  relocate(year, month) %>%
  select(-temporary)
```
...
exportM_pvL2 <- exportM_pvL2 %>%
  mutate(month = factor(month
                        (exportM_pvL2$`period`),
                        levels = 1:12, 
                        labels = month.abb, 
                        ordered = TRUE)) %>%
  mutate(year = as.character(year(ymd(exportM_pvL2$`period`)))) %>%
  relocate(year, month) %>%
  select(-temporary)
```
...
```{r}
tradesK2 <- importK_pvL2 %>% 
  full_join(exportK_pvL2)
```
...
unique(noExport2$country)
```

#### 3.3.5.3 Trade data at regions level

```{r}
tradesM2 <- importM_pvL2 %>% 
  full_join(exportM_pvL2) %>%
  rename(`region` = 3) %>%
...
| **2022 Apr 26**                                                                                                                                                                                      |
+======================================================================================================================================================================================================+
| **Singapore Disease Outbreak Response System Condition (DORSCON) Orange to Yellow**[^1]                                                                                                              |
|                                                                                                                                                                                                      |
...
[^1]: Ministry of Health. 2022. Further easing of community and border measures. https://www.moh.gov.sg/news-highlights/details/further-easing-of-community-and-border-measures
...
|                                                                                                                                                                                                                            |
| President Putin announced the begin of Russian invasion through Ukraine's eastern Donbas region.[^2]                                                                                                                       |
+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
:::

[^2]: 2022\. As Security Council meets on Ukraine crisis, Russia announces start of 'special military operation'. UN News. https://news.un.org/en/story/2022/02/1112592
